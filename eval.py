...
def parse_args(argv=None):
    ...
    # 对于函数add_argument()第一个是选项，第二个默认值，第三个是数据类型，第四个是help命令时的说明
    parser.add_argument('--trained_model',
                        default='weights/ssd300_mAP_77.43_v2.pth', type=str,
                        help='Trained state_dict file path to open. If "interrupt", this will open the interrupt file.')
    # ???? 这貌似是个很重要的参数，不知道是什么意思
    # 进一步限制要解析的预测数
    ...
    # 如果使用lincomb masks，则显示这些mask是如何生成的
    parser.add_argument('--display_lincomb', default=False, type=str2bool,
                        help='If the config uses lincomb masks, output a visualization of how those masks are created.')
    # 搞不懂怎么 运行显示模式，但不显示图像,无参数
    ...
    # 输出scripts、compute mask脚本中的内容，无参数
    ...
    # 仅在display模式下： 设置显示阈值，超过阈值的物体检测显示
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    with timer.env('Postprocess'):
        ...
        # 显示中间处理过程，返回检测到的所有 class mask boxes score
    ...
    with timer.env('Copy'):
        print("2 env copy")
        # t[1]就表示scores
        # argsort函数　返回数组排序后的索引值
        # descending=True　按降序排序，为false时按升序排序
        # 然后截取前　top_k个数
        # top_k作为一次保留的分数大小
        # idx 其实是一个下标数组，保存了score是前top_k大的下标
        # idx是gpu形式的tensor变量 [0, 1, 2]

        # 新增　改变识别种类
        idx = t[1].argsort(0, descending=True)[:args.top_k].cpu().numpy()

        list = ['person', 'bicycle', 'car', 'motorcycle', 'bus', 'truck', 'cat', 'dog']
        dy_idx = []
        for k in range(idx.shape[0]):
            # print(idx.shape[0])
            objClass = t[0][k].cpu().numpy()
            if cfg.dataset.class_names[objClass] in list:
                # idx = np.delete(idx, k)
                dy_idx.append(k)
        if not dy_idx:
            pixel = np.array([0, 0, 0])
            # print('shape', img_numpy.shape)
            [rows, cols, color] = img_gpu.shape
            mask_txt = np.zeros((rows, cols), dtype=np.int)
            return (img_gpu * 255).byte().cpu().numpy(), mask_txt, location
        ...
        if cfg.eval_mask_branch:
            ...
            # mask 不需要转换格式，因为是在GPU上绘制的
            # 原始的mask维度为（1, 768, 1024）
            # 只有mask不转换为cpu形式，因为后面还要进行矩阵操作
            masks = t[3][idx]
        #   截取下标对应的
        # classes scores boxes　都不再进行矩阵转换，所以转换成cpu形式
        classes, scores, boxes = [x[idx].cpu().numpy() for x in t[:3]]
        # print(classes)
        # print('score', scores)
        # print('box', boxes)
    #  ???? 这里又是什么意思　小于阈值了不是应该丢弃吗，赋值又是什么意思
    # numpy 创建的数组都有一个shape属性，这是一个元组，返回各个维度的维数
    # num_dets_to_consider　是本次要操作的大小
    num_dets_to_consider = min(args.top_k, classes.shape[0])
    # 只找到前top_k大的score还不行，还要判断score有没有大于阈值
    ...
    # 快速选择每个索引的颜色，并跟踪gpu中已经有的颜色，实现快速索引
    def get_color(j, on_gpu=None):
        global color_cache
        # 与c++中的问号运算符一致，如果有 class_color  则执行 classes[j]*5， 否则执行 j * 5
        # j*5再对COLORS的长度取余
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        # cat的意思是将两个张量（tensor）拼接在一起
        # view用于返回数据不变，但size不同的tensor，color原来的大小是三维，现在通过view增加一个轴，即变成了四维。
        # 将所有颜色拼接一起
        colors = torch.cat([get_color(j, on_gpu=img_gpu.device.index).view(1, 1, 1, 3) for j in range(num_dets_to_consider)], dim=0)
        masks_color = masks.repeat(1, 1, 1, 3) * colors * mask_alpha

        # This is 1 everywhere except for 1-mask_alpha where the mask is
        # × 矩阵逐元素相乘，除1-mask_alpha外，其他地方均为1
    ...
    # 将最终结果图片从 gpu 格式转换为 numpy，用CPU绘制
    img_numpy = (img_gpu * 255).byte().cpu().numpy()
    # print('gpu', img_gpu*255)
    # print('byte', (img_gpu * 255).byte())
    # print('cpu', (img_gpu * 255).byte().cpu())
    # print(img_numpy)
    ...
    if args.display_text or args.display_bboxes:
        # 相当于 for(let j = num_dets_to_consider; j--; j>0)
        for j in reversed(range(num_dets_to_consider)):
            x1, y1, x2, y2 = boxes[j, :]
            color = get_color(j)
            score = scores[j]
            obj = [cfg.dataset.class_names[classes[j]], score, x1, y1, x2, y2]
            location.append(obj)

            if args.display_bboxes:
                cv2.rectangle(img_numpy, (x1, y1), (x2, y2), color, 1)
                print(img_numpy.shape, x1, y1, x2, y2)
    ...
    print(img_numpy.shape[0], masks_color_summand.shape[0], location)
    return img_numpy, (masks_color_summand * 255).byte().cpu().numpy(), location
...
# 处理图片
def evalimage(net:Yolact, path:str, save_path:str=None, mask_path:str=None, mask_img_path:str=None, location_path:str=None):
    # 从这里才开始读入图片
    frame = torch.from_numpy(cv2.imread(path)).cuda().float()
    batch = FastBaseTransform()(frame.unsqueeze(0))
    preds = net(batch)
    # 所以是在这里进行检测　画图
    img_numpy, mask_img, location = prep_display(preds, frame, None, None, undo_transform=False)
    ...
    weidu = min(mask_img.shape)
    [rows, cols, color] = img_numpy.shape
    mask_txt = np.zeros((rows, cols), dtype=np.int)
    if weidu == 3:
        for i in range(rows):
            for j in range(cols):
                if not (mask_img[i, j] == pixel).all():
                    mask_txt[i, j] = 1
                # print(img_numpy[i, j])
    # print('mask_shape', mask_txt.shape)

    # mask_img = np.zeros((rows, cols, 3), dtype=np.int)
    # for i in range(rows):
    #     for j in range(cols):
    #         for k in range(3):
    #             if mask_txt[i, j]:
    ...
    if save_path is None:
        ...
        # 否则把图片写入指定路径
        if weidu == 3:
            np.savetxt(mask_path, mask_txt, fmt="%s")
            np.savetxt(location_path, location, fmt="%s")
            # save_path = save_path.join('.png')
...
def evalimages(net:Yolact, input_folder:str, output_folder:str):
    ...
    for p in Path(input_folder).glob('*'): 
        ...
        mask_name = '.'.join(name.split('.')[:-1]) + '.txt'
        mask_img_name = '.'.join(name.split('.')[:-1]) + '.jpg'
        location_name = '.'.join(name.split('.')[:-1]) + 'l' + '.txt'
        out_path = os.path.join(output_folder, name)
        mask_path = os.path.join(output_folder, mask_name)
        mask_img_path = os.path.join(output_folder, mask_img_name)
        location_path = os.path.join(output_folder, location_name)
        # out_path是存储原始图片的路径
        # mask_path存储文本掩码
        # mask_img_path存储掩码图片
        print("process", name)
        evalimage(net, path, out_path, mask_path, mask_img_path, location_path)
        # print(path + ' -> ' + out_path)
    print('Done.')
...
def evaluate(net:Yolact, dataset, train_mode=False):
    ...
    if args.image is not None:
        ...
        #  执行完对图片的操作就已经return了，后面的一大堆代码不用再考虑
        return
...
# 当以eval.py为执行入口时，这里就是入口函数（因为此时该模块__name__ 等于 __main__）
# 1.程序入口
# 2.作为模块导入时，跳过这一部分
